...
class _SettingsPageState extends State<SettingsPage> with WidgetsBindingObserver {
  ...
  Future<bool> _startCameraForMonitoring(double divisor) async {
    try {
      ...
      debugPrint(
        '[SettingsPage] CameraService started from visible UI isolate.',
      );
      ...
      debugPrint(
        '[SettingsPage] CameraService start failed: $error',
      );
      debugPrintStack(stackTrace: stackTrace);
      return false;
    }
  ...
  Future<void> _stopNativeCamera() async {
    try {
      ...
      debugPrint(
        '[SettingsPage] CameraService stop failed: $error',
      );
      debugPrintStack(stackTrace: stackTrace);
    }
  }
  ...
  Future<void> _toggleMonitoring(bool value) async {
    ...
    try {
      ...
      if (!monitoringPermissions) {
        _toast(
          'برای پایش وضعیت بدن، اجازه استفاده از دوربین را فعال کنید.',
        );
        return;
      }
      ...
      if (!cameraStarted) {
        _toast(
          'سرویس دوربین نتوانست شروع شود. مجوز دوربین را بررسی کنید.',
        );
        return;
      }
  ...
  Future<void> _toggleSound(bool value) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => soundEnabled = previous);
      }

      _toast('تغییر تنظیم صدای هشدار انجام نشد.');
    } finally {
      if (mounted) {
        setState(() => busySound = false);
      }
    }
  }
