...
class _SettingsPageState extends State<SettingsPage> with WidgetsBindingObserver {
  ...
  Widget _card(Widget child) => Container(padding: const EdgeInsets.all(17), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: line), boxShadow: [BoxShadow(color: Colors.black.withOpacity(.03), blurRadius: 18, offset: const Offset(0, 6))]), child: child);
  ...
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(backgroundColor: bg, body: SafeArea(child: loading ? const Center(child: CircularProgressIndicator(color: green)) : ListView(padding: const EdgeInsets.fromLTRB(18, 10, 18, 30), children: [
    ...
    _card(Column(children: [Row(children: [_icon(Icons.monitor_heart_outlined, green), const SizedBox(width: 11), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('پایش وضعیت بدن', style: TextStyle(color: text, fontSize: 13, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('گردن، قوز، مچ، فاصله و نور محیط', style: TextStyle(color: sub, fontSize: 10))])), Switch.adaptive(value: monitoring, onChanged: busyMonitoring ? null : _toggleMonitoring, activeColor: green)]),
      if (monitoring) ...[const Padding(padding: EdgeInsets.symmetric(vertical: 15), child: Divider(color: line, height: 1)), const Align(alignment: Alignment.centerRight, child: Text('نحوه دریافت هشدار', style: TextStyle(color: text, fontSize: 11, fontWeight: FontWeight.w800))), const SizedBox(height: 9), Row(children: [_modeButton(MonitoringMode.passive, Icons.visibility_off_outlined, 'پس‌زمینه', 'بدون شناور'), const SizedBox(width: 8), _modeButton(MonitoringMode.overlay, Icons.picture_in_picture_alt_outlined, 'شناور', 'روی صفحه')])],
    ])),
    _section('محافظت دیجیتال', 'محافظت از تجربه استفاده از صفحه را مستقل از پایش بدن کنترل کن.'),
    _card(Row(children: [_icon(Icons.shield_outlined, teal), const SizedBox(width: 11), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('پایش محتوای نامناسب', style: TextStyle(color: text, fontSize: 13, fontWeight: FontWeight.w900)), SizedBox(height: 4), Text('در صورت تشخیص، هشدار محافظتی روی صفحه نمایش داده می‌شود.', style: TextStyle(color: sub, fontSize: 10, height: 1.5))])), Switch.adaptive(value: nsfw, onChanged: busyNsfw ? null : _toggleNsfw, activeColor: teal)])),
  ...
  Widget _modeButton(MonitoringMode value, IconData icon, String title, String subtitle) { final selected = mode == value; return Expanded(child: InkWell(onTap: () => _setMode(value), borderRadius: BorderRadius.circular(15), child: AnimatedContainer(duration: const Duration(milliseconds: 180), padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: selected ? mint : bg, borderRadius: BorderRadius.circular(15), border: Border.all(color: selected ? green : line)), child: Row(children: [Icon(icon, color: selected ? green : sub, size: 18), const SizedBox(width: 7), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: text, fontSize: 10, fontWeight: FontWeight.w900)), const SizedBox(height: 2), Text(subtitle, style: const TextStyle(color: sub, fontSize: 8))]))])))); }
  Widget _action(IconData icon, String title, String subtitle, VoidCallback onTap) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(14), child: Row(children: [_icon(icon, green), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: text, fontSize: 11, fontWeight: FontWeight.w900)), const SizedBox(height: 3), Text(subtitle, style: const TextStyle(color: sub, fontSize: 9))])), const Icon(Icons.chevron_left_rounded, color: sub)]));
}
