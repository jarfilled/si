...
class _PostureAnalysisPageState extends State<PostureAnalysisPage> {
  ...
  void _listenToService() {
    _statusSubscription = BackgroundMonitorService.statusStream.listen((data) {
      ...
      final payload = data['metrics'] is Map ? Map<dynamic, dynamic>.from(data['metrics']) : data;
      setState(() {
        for (final key in live.keys) {
          final value = payload[key];
          if (value is num) live[key] = value.toInt();
          else if (value != null) live[key] = int.tryParse(value.toString()) ?? live[key]!;
        }
      });
    });
  }
  ...
  int _score() {
    ...
    if (stored > 0) return stored.round().clamp(0, 100);
    return (100 - _todayRisk() * .8).round().clamp(0, 100);
  }

  Widget _card({required Widget child, EdgeInsets padding = const EdgeInsets.all(17)}) => Container(
    padding: padding,
    decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: line), boxShadow: const [BoxShadow(color: Color(0x08000000), blurRadius: 18, offset: Offset(0, 6))]),
    child: child,
  );
  ...
  Widget _liveCard() {
    final total = live.values.fold<int>(0, (sum, value) => sum + value);
    return Container(
      padding: const EdgeInsets.all(17),
      decoration: const BoxDecoration(gradient: LinearGradient(colors: [green, teal]), borderRadius: BorderRadius.all(Radius.circular(22))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [const Icon(Icons.radar_rounded, color: Colors.white, size: 23), const SizedBox(width: 8), const Expanded(child: Text('پایش زنده', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w900))), Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle))]),
        const SizedBox(height: 6),
        Text(total == 0 ? 'هنوز هشدار زنده‌ای ثبت نشده.' : 'در حال ثبت الگوهای بدنی و فاصله از صفحه.', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white70, fontSize: 10)),
  ...
  Widget _livePill(String title, int value, IconData icon) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 8),
    decoration: BoxDecoration(color: Colors.white.withValues(alpha: .15), borderRadius: BorderRadius.circular(14)),
    child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(icon, color: Colors.white, size: 15), const SizedBox(width: 5), Text('$title ${value}s', style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))]),
  );
  ...
  Widget _smallMetric(String title, String value, IconData icon, Color color) => Container(
    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 11),
    decoration: BoxDecoration(color: background, borderRadius: BorderRadius.circular(15)),
    child: Row(children: [Icon(icon, color: color, size: 17), const SizedBox(width: 7), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: muted, fontSize: 8)), const SizedBox(height: 2), Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: text, fontSize: 11, fontWeight: FontWeight.w900))]))]),
  );

  Widget _chart() {
    if (daily.length < 2) return _card(child: const Row(children: [Icon(Icons.show_chart_rounded, color: teal), SizedBox(width: 9), Expanded(child: Text('با چند روز داده، روند هفتگی اینجا نمایش داده می‌شود.', style: TextStyle(color: muted, fontSize: 11)))]));
    final spots = <FlSpot>[];
    for (var i = 0; i < daily.length; i++) {
      final row = daily[i];
      final risk = _number(row, 'neck') + _number(row, 'hunch') + _number(row, 'wrist');
      ...
      SizedBox(height: 190, child: LineChart(LineChartData(
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 26, interval: 1, getTitlesWidget: (value, meta) {
            ...
            final date = DateTime.parse(daily[i]['date'].toString());
