...
class _ProfilePageState extends State<ProfilePage> {
  ...
  String _string(dynamic value, {String fallback = ''}) {
    ...
    return valueString == null || valueString.isEmpty ? fallback : valueString;
  }
    ...
    child: Column(children: [
      Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(displayName, style: const TextStyle(color: text, fontSize: 20, fontWeight: FontWeight.w900)),
          const SizedBox(height: 5),
          Text(username.isEmpty ? 'پروفایل شخصی' : '@$username', style: const TextStyle(color: subtext, fontSize: 12)),
        ])),
      ...
      Row(children: [
        Expanded(child: _miniInfo(Icons.cake_outlined, age > 0 ? '$age سال' : 'ثبت نشده', 'سن')),
        Container(width: 1, height: 34, color: line),
        Expanded(child: _miniInfo(Icons.wc_outlined, gender == 'female' ? 'خانم' : 'آقا', 'جنسیت')),
        Container(width: 1, height: 34, color: line),
        Expanded(child: _miniInfo(Icons.tune_rounded, hunchDivisor > 0 ? 'فعال' : 'نیازمند', 'کالیبراسیون')),
      ]),
    ]),
  );
  ...
  Widget _healthSnapshot() {
    ...
    return Row(children: [
      Expanded(child: _metric('امتیاز امروز', '$score', Icons.favorite_rounded)),
      const SizedBox(width: 8),
      Expanded(child: _metric('صفحه', '${screen}د', Icons.phone_android_rounded)),
      const SizedBox(width: 8),
      Expanded(child: _metric('وضعیت بدن', '${posture}د', Icons.accessibility_new_rounded)),
    ]);
  }
  ...
  Widget _moodCard() {
    ...
    return _card(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 5),
      const Text('یک ثبت کوتاه برای دنبال کردن حال روزانه.', style: TextStyle(color: subtext, fontSize: 11)),
      const SizedBox(height: 12),
      Wrap(spacing: 6, runSpacing: 6, children: moods.entries.map((e) => ChoiceChip(selected: mood == e.key, label: Text('${e.value} ${e.key}'), onSelected: (_) async { setState(() => mood = e.key); final prefs = await SharedPreferences.getInstance(); await prefs.setString('si_mood_${DateTime.now().year}_${DateTime.now().month}_${DateTime.now().day}', e.key); }, selectedColor: mint, side: BorderSide(color: mood == e.key ? green : line))).toList()),
    ]));
  }
  ...
  Widget build(BuildContext context) {
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(
      backgroundColor: bg,
      body: SafeArea(child: loading ? const Center(child: CircularProgressIndicator(color: green)) : RefreshIndicator(
        color: green,
        onRefresh: _load,
        child: ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.fromLTRB(18, 10, 18, 30), children: [
          Row(children: [
            const Text('پروفایل', style: TextStyle(color: text, fontSize: 23, fontWeight: FontWeight.w900)),
            const Spacer(),
          ...
          _healthSnapshot(),
          const SizedBox(height: 18),
          _sectionTitle('سلامت شخصی', 'یک فضای کوچک برای بررسی وضعیت ذهنی روزانه.'),
          _moodCard(),
          const SizedBox(height: 18),
